...
public class Skeleton_agent : Training_scripts
{
    ...
    public override void OnActionReceived(ActionBuffers vectorAction)
    {
        ...
        if(desicion_Delay == 0){
            Ai_scripts.desx = vectorAction.ContinuousActions[0] >= 0 ? Mathf.Max(vectorAction.ContinuousActions[0], 0.3f) : Mathf.Min(vectorAction.ContinuousActions[0], -0.3f);
            if(distance < 5f) Ai_scripts.desy = vectorAction.ContinuousActions[1] >= 0 ? Mathf.Max(vectorAction.ContinuousActions[1], 0.3f) : Mathf.Min(vectorAction.ContinuousActions[1], -0.3f);
        ...
        if(Ai_scripts.iFrame == 0){
            ...
            if(distance <= 2 && action > 1 && action < 5)
                AddReward(30);
            if(Player_weapon.sweaping && action == 1)
                AddReward(60);
            if(distance < 5 && action == 5)
                AddReward(20);
